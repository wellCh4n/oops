...
1. **fetch** (`CloneContainer`): Clones source code from git (or downloads ZIP via curl for ZIP source type). A Git clone then prints the commit it checked out (`git log -1`, chained with `&&` so a failed clone still fails the step) — a publish names only a branch, so the log is where the resolved commit is recorded
...
**Build variables**: each environment's build config carries `buildVariables` (`List<BuildVariable>` of `name`/`value`, inside the `environment_configs` JSON blob next to `buildCommand`). They reach the build two ways: `BaseContainer.addBuildVariables` sets them as environment variables on `fetch`, `compile` and `publish` — placed *before* the step's own variables, because Kubernetes lets the last entry of a repeated name win, so a variable called `GIT_SSH_COMMAND` or `REGISTRY_AUTH_FILE` cannot take the step's wiring away — and `PublishContainer` additionally passes each as `--build-arg`, since the environment of the buildah process never reaches a Dockerfile `RUN` (the Dockerfile must declare a matching `ARG`). The values travel as trailing positional arguments that the script folds into `"$@"`, never as shell text. They are **not secrets**: a build arg stays readable in the image history, which is why the editor shows values as typed and the build summary prints them. Names are validated in `ApplicationBuildConfigPolicy.normalizeBuildVariables` (shell identifier grammar, unique per environment). In `PUT .../build/config` an environment entry whose `buildVariables` is omitted keeps its stored variables — the same "absent means unchanged" as the list itself, one level down — so the CLI's `--build-command` cannot wipe them; an explicit empty list clears them.

The job does not read them from the application: `DeploymentService` snapshots the environment's variables onto the pipeline as `build_config` (`PipelineBuildConfig`, a top-level record beside `PublishConfig` with its converter in `jpa/converter/`, the counterpart of `publish_config` — that one is what the operator chose at publish time, this one the slice of the application's build config that applied), and `PipelineExecuteTask` builds the containers from that snapshot. So the `done` step's build summary, which lists the variables with their values, shows what the build really ran with however the config has been edited since. The column is `NULL` for rollbacks, image publishes and pipelines older than V24; build image, build commands and the Dockerfile are still read live from the application and are not yet part of the snapshot.
...
- `ApplicationBuildConfig`: Stores source type (`GIT`/`ZIP`/`IMAGE`), repository or image name (`SourceConfig` JSON blob: `GitSourceConfig`, `ZipSourceConfig`, `ImageSourceConfig`), build image, per-environment build commands and build variables, and Dockerfile config (`BUILTIN` path or inline `USER` content). Frontend: `application-build-info.tsx` (the variable rows are `build-variables-editor.tsx`; the environment selector sits above both the variables and the build command, since both belong to the environment it shows); the publish page renders the IMAGE tag as a prefixed input (`components/ui/input-group.tsx`) with the image name fixed in front of it.
...
`resources/inotify-limits.yaml` is a DaemonSet that raises `fs.inotify.max_user_instances` (default 128) on every node. Each followed container log costs the kubelet one inotify instance, and the count is per uid, so every root process on the node draws on the kubelet's pool — each `containerd-shim` holds two, so a node running a few hundred containers spends the default on shims alone; once it is spent every log request answers `failed to create fsnotify watcher: too many open files`. Note that with the Vert.x HTTP client `LogWatch.close()` does **not** release that instance — only closing the `KubernetesClient` that owns the connection does — which is why every log-following gateway opens a client of its own per stream and closes it with the stream.
...
- **The status stream drives the step bar**, not the log. The frontend (`apps/[namespace]/[name]/pipelines/[pipelineId]/page.tsx`) shows one step's log at a time: by default the running step (following it to the next when it finishes), else the failed one, else the last that ran; clicking a step pins it, and a "follow latest" button unpins. The trailing `done` container (`echo done!`) is a step like any other in the bar, but its log says nothing, so viewing it — by following a finished build or by clicking it — shows a build summary in the log panel instead: duration, artifact, per-step timings, then the pipeline's phase (deploying / rolled out / awaiting a manual deploy, where the deploy button lives; the header keeps that button only for a pipeline without a build, i.e. an image publish). A `PENDING` step under a finished `phase` never ran.
