...
**Everything degrades to plain exec.** Read-only filesystem, missing `cat`, unsupported architecture (only amd64/arm64 are bundled), a `/tmp` mounted `noexec`, `oops.terminal.resume-enabled: false`, or an unauthenticated socket — all fall back to the shell OOPS always opened, with client-side reconnect still working. A terminal never fails just because it cannot be made resumable.

That rests on one rule: **every gate runs the binary (`dtach --version`), never `test -x`** — in `DtachInstaller`'s probe, again after the upload, and once more in the gateway's terminal script. The execute bit is set in every way this can fail (a `noexec` mount, an LSM denying exec by path, a wrong-architecture binary, or a `cat` that saw stdin end early and left a truncated ELF), and the exit status gives nothing away either, since a truncated ELF segfaults with 139 rather than returning 126. Getting this wrong does not merely disable resume: the terminal script `exec`s dtach, so a binary that will not load takes the shell with it, leaving nothing to fall back to and a browser that reconnects into the same failure forever. `DtachState` therefore has three values, not two — a dtach that is present but unrunnable is *not* treated as absent, because re-uploading 50KB on every connect is unbounded when the client reconnects on its own; the install writes `/tmp/.oops/dtach.unusable` and later terminals go straight to a plain shell.
