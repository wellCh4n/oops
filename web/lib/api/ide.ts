...
export const listIDEs = async (namespace: string, application: string, env: string): Promise<ApiResponse<IDEInstance[]>> => {
  const response = await apiFetch(`/api/namespaces/${namespace}/applications/${application}/ides?environment=${env}`)
  if (!response.ok) {
...
export const getDefaultIDEConfig = async (namespace: string, application: string, env: string): Promise<ApiResponse<IDEDefaultConfig>> => {
  const response = await apiFetch(`/api/namespaces/${namespace}/applications/${application}/ides/config/default?environment=${env}`)
  if (!response.ok) {
...
export const deleteIDE = async (namespace: string, application: string, name: string, env: string): Promise<void> => {
  const response = await apiFetch(`/api/namespaces/${namespace}/applications/${application}/ides/${name}?environment=${env}`, {
    method: "DELETE",
  })
  if (!response.ok) {
