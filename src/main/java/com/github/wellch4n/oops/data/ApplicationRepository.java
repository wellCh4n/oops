...
public interface ApplicationRepository extends JpaRepository<Application, String>, JpaSpecificationExecutor<Application> {
    ...
    @Query("SELECT a FROM Application a WHERE (:namespace = 'all' OR a.namespace = :namespace) AND LOWER(a.name) LIKE LOWER(CONCAT('%', :keyword, '%')) AND (:ownerId IS NULL OR a.owner = :ownerId) ORDER BY CASE WHEN a.owner = :currentUserId THEN 0 ELSE 1 END, a.createdTime DESC")
