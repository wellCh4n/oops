...
public class BuildStorageVolume {
    ...
    public BuildStorageVolume(List<BuildStorage> buildStorages) {

        for (BuildStorage buildStorage : buildStorages) {
            V1PersistentVolumeClaimVolumeSource pvc = new V1PersistentVolumeClaimVolumeSource();
            pvc.claimName(buildStorage.getPvcName());
            V1Volume buildCache = new V1Volume().name(buildStorage.getPvcName()).persistentVolumeClaim(pvc);
            this.volumes.add(buildCache);

            V1VolumeMount buildCacheMount = new V1VolumeMount()
                    .name(buildStorage.getPvcName())
                    .mountPath(buildStorage.getPath());
