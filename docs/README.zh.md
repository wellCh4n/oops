# OOPS
> Kubernetes Is All You Need

<img src="../web/public/icon.png" width="120" alt="OOPS logo">

OOPS 是一个 Kubernetes PaaS，**人和 AI Agent 都是平等的操作者**。部署应用、管理多集群、运行沙箱命令、配置域名——既能在简洁的 Web UI 中完成，也可以通过内置的 [OOPS CLI Skill](../skills/oops/SKILL.md) 由程序驱动。

[English](../README.md)

[![React Doctor](https://www.react.doctor/share/badge?p=oops&s=84&w=232&f=60)](https://www.react.doctor/share?p=oops&s=84&w=232&f=60)

## 为什么选择 OOPS？

KubeSphere、Rainbond、ArgoCD 各自解决了 Kubernetes 的一块真实问题——但它们都默认**人**是唯一的操作者。OOPS 保留完整的用户体验，同时让 Agent 也能用同一套接口操作平台：

| 项目 | 服务对象 | OOPS 的差别 |
|---|---|---|
| **KubeSphere** | 用插件生态搭建企业 IDP 的平台团队 | 接口少而聚焦——同一个动作在 UI 和 API 上都能完成，不用研究插件框架 |
| **Rainbond** | 不想学习 Kubernetes 的开发者 | 对开发者同样简单，并额外提供 API，让 Agent 也能驱动 |
| **ArgoCD** | 把 Git 和集群保持一致的 GitOps 流水线 | 人或 Agent 都能直接通过代码压缩包部署，Git 不是必须 |

## 功能特性

### 部署任意应用
推送代码或上传 ZIP，选择仓库里的 Dockerfile 或填写内联 Dockerfile，OOPS 会自动构建镜像并发布到 Kubernetes 集群，构建过程日志实时输出。可一键回滚到任意一次成功的构建——复用已有镜像、完全跳过构建阶段。

### 精细调整每次部署
按环境设置 CPU 与内存的 requests / limits、副本数、存活与就绪探针，并通过 ConfigMap 和环境变量注入配置——无需手写 YAML。还可按环境用 cron 选择器配置定时滚动重启。

### 多集群管理
可同时接入多个集群，在同一控制台中并排管理，随时切换部署目标，无需离开界面。需要重新组织时，可将应用迁移到其他命名空间。

### 团队与权限控制
可邀请多个用户，分配 ADMIN 或 USER 角色。每个应用都有归属者，可将访问权限分享给协作者，而删除等危险操作仅限归属者与管理员。

### 实时观察运行状态
实时日志、浏览器内终端、可编辑的 Pod 文件、Kubernetes 事件、集群节点、发布状态以及各 Pod 的 CPU / 内存用量一眼可见。

### 流水线通知
构建与部署状态可直接推送到聊天——已创建、构建成功、部署中、成功、失败、已停止。目前通过飞书（Lark）送达。

### 给 Agent 用的沙箱
内置 Agent 级沙箱能力（类似 [OpenSandbox](https://github.com/alibaba/OpenSandbox)）——让 Agent 在隔离环境里跑命令、装依赖、编辑文件、调试问题。既支持一次性流式执行，也支持带浏览器终端的长驻沙箱实例。

### 面向自动化的 OpenAPI
使用每个用户自己的 Access Token 调用 `/openapi/**`，也可以让 Agent 通过 [skills/oops](../skills/oops/SKILL.md) 里的 Python CLI 驱动部署流程。UI 和自动化使用的是同一套平台能力。

### IDE 与静态资产
为应用启动可选的 code-server IDE 实例，并在 UI 中管理基于对象存储的静态资产。

### 多域名管理
在统一的控制台中管理多个域名，支持自动签发 HTTPS 证书或上传自有 TLS 证书，同时支持通配符域名。

## 环境要求

| 组件 | 是否必需 | 用途 |
|---|---|---|
| Kubernetes 集群（1.22+） | 是 | 运行应用、流水线和 IDE |
| MySQL 8.x | 是 | 持久化 OOPS 元数据 |
| 容器镜像仓库 | 是 | 流水线镜像推送 / 拉取 |
| Traefik v3 | 否 | Ingress 与 HTTPS 路由 |
| S3 兼容对象存储 | 否 | ZIP 源码上传与静态资产 |

## 快速开始

```bash
cp docker/application.yml.example docker/application.yml
# 编辑 docker/application.yml — 修改 secret、数据源以及可选功能
```

### Docker Compose（预构建镜像）

从 GitHub Container Registry 拉取 `ghcr.io/wellch4n/oops-backend:latest` 与 `ghcr.io/wellch4n/oops-frontend:latest`。

```bash
docker compose -f docker/docker-compose.yml up -d
```

打开 <http://localhost:8080>，使用 `admin` / `admin123` 登录。

### Docker Compose（源码构建）

本地构建镜像，不从 GHCR 拉取。

自带 MySQL：

```bash
docker compose -f docker/docker-compose.build.yml up -d --build
```

已有 MySQL — 先在 `docker/application.yml` 的 `spring.datasource` 段修改连接信息：

```bash
docker compose -f docker/docker-compose.local.yml up -d --build
```

### 从源码运行

```bash
# 后端
cp config/application.yml.example config/application.yml
./mvnw spring-boot:run

# 前端
cd web && pnpm install && pnpm dev
```

默认管理员凭证：`admin` / `admin123`（在 `application.yml` 中修改 `oops.admin.password` 覆盖）。

## 工作原理

### 应用构建与部署流水线

```mermaid
flowchart TD
    Start([用户触发部署]) --> Src{源码类型}
    Src -->|GIT| Clone["① 克隆<br/>git clone"]
    Src -->|ZIP| Upload["上传 ZIP<br/>（预签名 S3 URL）"]
    Upload --> CloneZ["① 克隆<br/>curl 从 S3 下载"]
    Clone --> Dockerfile["② Dockerfile<br/>（可选内联 USER Dockerfile）"]
    CloneZ --> Dockerfile
    Dockerfile --> Build["③ 构建<br/>（可选命令）"]
    Build --> Push["④ 推送<br/>Buildah 构建并推送"]
    Push --> Scan{"Job 结果<br/>（5s 轮询）"}
    Scan -->|失败| Err([错误 ❌])
    Scan -->|成功| Mode{部署模式}
    Mode -->|手动| Wait([BUILD_SUCCEEDED ⏸️<br/>等待手动触发])
    Mode -->|立即| Deploy["部署 K8s 资源<br/>StatefulSet · Service · IngressRoute"]
    Wait --> Deploy
    Deploy --> Rollout{"发布健康检查<br/>（5s 轮询）"}
    Rollout -->|就绪| Ok([部署成功 ✅])
    Rollout -->|失败 / 超时| Err

    style Start fill:#4CAF50,color:#fff
    style Ok fill:#4CAF50,color:#fff
    style Err fill:#f44336,color:#fff
    style Wait fill:#FF9800,color:#fff
```

### 流水线状态机

```mermaid
stateDiagram-v2
    direction TB
    [*] --> INITIALIZED: 流水线创建
    INITIALIZED --> RUNNING: K8s Job 提交
    INITIALIZED --> DEPLOYING: 回滚跳过构建

    RUNNING --> BUILD_SUCCEEDED: Job 成功<br/>(手动模式)
    RUNNING --> DEPLOYING: Job 成功<br/>(立即部署模式)
    RUNNING --> ERROR: Job 失败
    RUNNING --> STOPPED: 用户取消

    BUILD_SUCCEEDED --> DEPLOYING: 用户触发部署
    BUILD_SUCCEEDED --> STOPPED: 用户取消

    DEPLOYING --> ROLLING_OUT: K8s 资源已应用
    DEPLOYING --> ERROR: 部署失败
    DEPLOYING --> STOPPED: 用户取消

    ROLLING_OUT --> SUCCEEDED: 发布就绪
    ROLLING_OUT --> ERROR: 发布失败

    SUCCEEDED --> [*]
    ERROR --> [*]
    STOPPED --> [*]
```

## 截图

![applications.png](../snapshots/applications.png)

![app_deploy.png](../snapshots/app_deploy.png)

![pod_status.png](../snapshots/pod_status.png)

![pod_terminal.png](../snapshots/pod_terminal.png)

![env_manager.png](../snapshots/env_manager.png)

![quick_command.png](../snapshots/quick_command.png)

![web_ide.png](../snapshots/web_ide.png)

## 许可证

本项目基于 Apache License 2.0 开源。详见 [LICENSE](../LICENSE) 文件。
